...
function mapGuildToArea(guildId: string): string | null {
    ...
    if (exact)
        return normalizeAreaName(exact.name);
    if (cfg.banca && cfg.banca.supportGuildId === guildId)
        return 'Suporte';
    return null;
}
export default { data: new SlashCommandBuilder().setName('ranking').setDescription('Enviar ranking da área em um canal')
        ...
        if (!interaction.guild) {
            await interaction.editReply('Use em um servidor.');
            return;
        }
        const area = mapGuildToArea(interaction.guild.id);
        if (!area) {
            await interaction.editReply('Este servidor não está mapeado para uma área no config.');
            return;
        }
