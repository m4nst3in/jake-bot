...
import { loadPunishmentConfig, hasPermissionToPunish, getUserHighestRank, formatDuration, isStaffMember } from '../utils/punishment.ts';
export default {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields({ name: '<a:mov_call1:1252739847614103687> Usuário', value: `<@${target.id}>\n\`${target.id}\``, inline: true }, { name: '<a:mov_call1:1252739847614103687> Patente', value: userRank.rankName, inline: true }, { name: '<a:mov_call1:1252739847614103687> Entrou no Servidor', value: joinedAt ? `<t:${Math.floor(joinedAt.getTime() / 1000)}:D>` : 'Desconhecido', inline: true })
                .setFooter({ text: 'Sistema de Punições - CDW', iconURL: interaction.guild?.iconURL() || undefined })
                .setTimestamp();
            if (activePunishments.length > 0) {
                embed.setColor(0xE74C3C);
                embed.addFields({ name: '<a:mov_call1:1252739847614103687> Punições Ativas', value: punishmentDetails.join('\n'), inline: false }, { name: '<a:mov_call1:1252739847614103687> Total de Punições', value: `${activePunishments.length} ativa(s)`, inline: true });
            }
            else {
                embed.setColor(0x2ECC71);
                embed.addFields({ name: '<a:mov_call1:1252739847614103687> Status', value: 'Nenhuma punição ativa', inline: false });
            }
            embed.addFields({ name: '<a:mov_call1:1252739847614103687> Conta Criada', value: `<t:${Math.floor(accountCreated.getTime() / 1000)}:D>`, inline: true }, { name: '<a:mov_call1:1252739847614103687> Verificado por', value: `<@${executor.id}>`, inline: true });
            await interaction.reply({
                embeds: [embed],
                ephemeral: true
            });
