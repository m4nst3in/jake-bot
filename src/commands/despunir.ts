import { SlashCommandBuilder, ChatInputCommandInteraction, EmbedBuilder, PermissionFlagsBits, GuildMember, ButtonBuilder, ButtonStyle, ActionRowBuilder, ComponentType } from 'discord.js';
import { loadConfig } from '../config/index.ts';
import { logger } from '../utils/logger.ts';
import { loadPunishmentConfig, hasPermissionToPunish, canPunishTarget, canRemovePunishment } from '../utils/punishment.ts';
        ...
        try {
            ...
            if (activePunishments.length === 0) {
                const embed = new EmbedBuilder()
                    ...
                    .addFields({ name: '<a:setabranca:1417092970380791850> Usuário', value: `<@${target.id}>`, inline: true }, { name: '<a:setabranca:1417092970380791850> Verificado por', value: `<@${executor.id}>`, inline: true })
                    .setColor(0x95A5A6)
            ...
            if (activePunishments.length === 1) {
                ...
                const embed = new EmbedBuilder()
                    ...
                    .addFields({ name: '<a:setabranca:1417092970380791850> Usuário', value: `<@${target.id}>`, inline: true }, { name: '<a:setabranca:1417092970380791850> Executor', value: `<@${executor.id}>`, inline: true }, { name: '<a:setabranca:1417092970380791850> Punição Removida', value: punishment.name, inline: false })
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields({ name: '<a:setabranca:1417092970380791850> Usuário', value: `<@${target.id}>`, inline: true }, { name: '<a:setabranca:1417092970380791850> Punições Ativas', value: activePunishments.map(p => `• ${p.name}`).join('\n'), inline: false })
                .setColor(0x3498DB)
                .setFooter({ text: 'Sistema de Punições - CDW', iconURL: interaction.guild?.iconURL() || undefined })
                .setTimestamp();
            ...
            try {
                ...
                const resultEmbed = new EmbedBuilder()
                    ...
                    .addFields({ name: '<a:setabranca:1417092970380791850> Usuário', value: `<@${target.id}>`, inline: true }, { name: '<a:setabranca:1417092970380791850> Executor', value: `<@${executor.id}>`, inline: true }, { name: '<a:setabranca:1417092970380791850> Punições Removidas', value: removedPunishments.join('\n') || 'Nenhuma', inline: false })
...
async function removePunishment(target: GuildMember, punishment: ActivePunishment, executor: GuildMember): Promise<boolean> {
    try {
...
async function logPunishmentRemoval(target: GuildMember, removedPunishments: string[], executor: GuildMember, interaction: ChatInputCommandInteraction) {
    try {
        const config = loadConfig() as any;
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields({ name: '<a:setabranca:1417092970380791850> Usuário', value: `<@${target.id}>\n\`${target.id}\``, inline: true }, { name: '<a:setabranca:1417092970380791850> Executor', value: `<@${executor.id}>\n\`${executor.id}\``, inline: true }, { name: '<a:setabranca:1417092970380791850> Punições Removidas', value: removedPunishments.join('\n'), inline: false }, { name: '<a:setabranca:1417092970380791850> Horário', value: `<t:${Math.floor(Date.now() / 1000)}:F>`, inline: true })
            .setFooter({ text: 'Sistema de Punições - CDW', iconURL: interaction.guild?.iconURL() || undefined })
            .setTimestamp();
