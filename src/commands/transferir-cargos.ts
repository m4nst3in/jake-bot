...
export default {
	...
	async execute(interaction: ChatInputCommandInteraction) {
		...
		try {
			...
			if (!origemMember) return interaction.editReply('Usuário de origem não encontrado ou fora do servidor.');
			const destinoMember = await guild.members.fetch(destinoId).catch(()=>null);
			if (!destinoMember) return interaction.editReply('Nova conta não encontrada no servidor principal.');
			...
			const logGuild = guild; // principal
			...
			const embed = new EmbedBuilder()
				...
				.addFields(
					{ name: '<a:star3:1120757490011877558> Origem', value: `<@${origemId}> (\`${origemId}\`)`, inline: true },
					{ name: '<a:star3:1120757490011877558> Destino', value: `<@${destinoId}> (\`${destinoId}\`)`, inline: true },
					{ name: '<a:star3:1120757490011877558> Cargos Transferidos', value: added.length ? added.map(id => `<@&${id}>`).join(' ') : 'Nenhum (já presentes ou bloqueados)', inline: false },
					  { name: '<a:star3:1120757490011877558> Cargos Ignorados', value: excluded.length ? excluded.map(id => `<@&${id}>`).join(' ') : 'Nenhum', inline: false },
					  { name: '<a:star3:1120757490011877558> Removidos da Origem', value: removed.length ? removed.map(id => `<@&${id}>`).join(' ') : 'Nenhum', inline: false }
				)
