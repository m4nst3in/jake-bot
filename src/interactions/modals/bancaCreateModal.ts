...
export default { id: 'banca_create_modal', async execute(interaction: ModalSubmitInteraction){
  ...
  if(!interaction.guild){ await interaction.editReply('Servidor indisponível'); return; }
  ...
  if(interaction.guild.id === DESIGN_GUILD_ID){ channelName = `${DESIGN_PREFIX}${sanitized}`; }
  ...
  // Recrutamento custom embed (novo layout) se for guild de recrutamento
