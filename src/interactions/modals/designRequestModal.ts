...
// Cargo de designers (aplicado a todos os tickets)
const DESIGN_ROLE_ID = '1183909149784952908';

export default {
  id: /design_request_modal:.+/, // inclui a área após ':'
  ...
  const texto = interaction.fields.getTextInputValue('design_texto').trim();
    ...
    if(!texto){ await interaction.editReply('Texto obrigatório.'); return; }
  ...
  channel = await interaction.guild.channels.create({ name: channelName, parent: categoryId, permissionOverwrites: overwrites });
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        { name: 'Feito por', value: `<@${interaction.user.id}>`, inline: true },
        { name: 'Texto na Arte', value: texto || '—', inline: true },
        { name: 'Dimensão', value: dimensao || '—', inline: true },
        { name: 'Cores', value: cores || '—', inline: true },
        { name: 'Descrição do pedido', value: descricao || '—', inline: false },
        { name: 'Data de Entrega', value: entrega || '—', inline: true }
      )
