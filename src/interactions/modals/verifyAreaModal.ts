...
export default {
  ...
  async execute(interaction: ModalSubmitInteraction) {
    ...
    // Validação de vínculo com cargo principal na guild primária
    ...
    // Mapa cargos principais (mesma definição anterior)
    const MAIN_AREA_ROLES: Record<string, { area: string }> = {
    ...
    if (assign.waitingRole) toRemove.push(assign.waitingRole);
    // Limpa possíveis restos da outra opção (garantir exclusividade)
    ...
    // Espelhamento de cargo de staff (patente) se possuir no servidor principal
    ...
    // Ordena por tamanho do nome para evitar matches incorretos? Aqui basta simples varredura
    for (const [rankName, mainRoleId] of Object.entries(mainRanks)) {
      if (rankName === 'staff') continue;
