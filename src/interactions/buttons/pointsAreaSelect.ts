...
export default {
    ...
    const modal = new ModalBuilder().setCustomId(`pts_amount:${mode}:${area}`).setTitle(`${mode === 'add' ? 'Adicionar' : 'Remover'} • ${area}`);
        ...
        try {
            // Pode falhar com 10062 (Unknown interaction) se o usuário clicou após expiração (>15m) ou token inválido.
            await interaction.showModal(modal);
        } catch (err: any) {
            if (err && err.code === 10062) {
                // Tenta avisar o usuário para reiniciar o fluxo
                try {
            ...
            // Outros erros silenciosamente ignorados ou podem ser logados se houver logger global
            try {
                if (!interaction.replied && !interaction.deferred) {
