...
export default {
    ...
    async execute(interaction: ButtonInteraction) {
        ...
        try {
            ...
            if (mode === 'inic') {
                if (!canSetIniciante(interaction.member as GuildMember))
                    return interaction.editReply('Sem permissão para definir Iniciante.');
                ...
                for (const [name, id] of Object.entries(cfg.roles || {})) {
                    ...
                    if (target.roles.cache.has(String(id))) {
                        try {
                            await target.roles.remove(String(id), 'Recrutamento: resetando hierarquia para Iniciante');
                        }
                        catch { }
                    }
                }
                if (inic && !target.roles.cache.has(inic)) {
                    await target.roles.add(inic, 'Recrutamento (Iniciante)').catch(() => { });
                    added.push(inic);
                }
                try {
                ...
                const embed = new EmbedBuilder()
                    ...
                    .addFields({ name: '<:branco_membros:1303749626062573610> Usuário', value: `<@${userId}>\n(${userId})`, inline: true }, { name: '<:crown2:1411488673924644944> Staff', value: `<@${interaction.user.id}>\n(${interaction.user.id})`, inline: true }, { name: '<a:staff_cdw:934664526639562872> Equipe', value: String(team || '').toUpperCase(), inline: true })
                    .addFields({
                ...
                try {
                    const logEmbed = new EmbedBuilder()
                        ...
                        value: (() => {
                            ...
                            if (inic)
                                r.push(`<@&${inic}>`);
                            if ((cfg as any).roles?.staff)
                                r.push(`<@&${(cfg as any).roles.staff}>`);
                            return r.join(' ');
                        })()
                    })
                        .setTimestamp();
            ...
            if (mode === 'team') {
                if (!canSetIniciante(interaction.member as GuildMember))
                    return interaction.editReply('Sem permissão para definir cargo de equipe.');
                const added: string[] = [];
                try {
                    const areaCfg = (cfg.areas || []).find((a: any) => a.name.toLowerCase() === team);
                    const primaryMap = cfg.primaryGuildTeamRoles || {};
                    ...
                    if (interaction.guildId === cfg.mainGuildId && primaryRoleId)
                        teamRoleId = primaryRoleId;
                    if (!teamRoleId)
                        return interaction.editReply('Cargo de equipe não configurado.');
            ...
            if (mode === 'mig') {
                if (!canSetWeeks(interaction.member as GuildMember))
                    return interaction.editReply('Sem permissão para migração.');
                const row = new ActionRowBuilder<ButtonBuilder>().addComponents(new ButtonBuilder().setCustomId(`recruit_mig_weeks:1:${team}:${userId}`).setLabel('1 Semana').setStyle(1), new ButtonBuilder().setCustomId(`recruit_mig_weeks:2:${team}:${userId}`).setLabel('2 Semanas').setStyle(1), new ButtonBuilder().setCustomId(`recruit_mig_weeks:3:${team}:${userId}`).setLabel('3 Semanas').setStyle(1), new ButtonBuilder().setCustomId(`recruit_mig_weeks:merit:${team}:${userId}`).setLabel('Mérito').setStyle(4));
