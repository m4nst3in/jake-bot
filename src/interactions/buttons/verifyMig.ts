...
export default {
  ...
  async execute(interaction: ButtonInteraction) {
    ...
    // Encontrar maior patente que o usuário possui (varremos do topo para baixo)
    ...
    const toAdd: string[] = [];
    // Sempre cargo base MIG
    if (!gm.roles.cache.has(MIG_MEMBER_ROLE)) toAdd.push(MIG_MEMBER_ROLE);
    ...
    if (matchedRankName) {
      ...
      if (mirrorRoleId) {
        if (!gm.roles.cache.has(mirrorRoleId)) toAdd.push(mirrorRoleId);
        mirroredRankApplied = matchedRankName;
      } else if (fallbackRoleId && matchedIdx !== -1 && subCmdIdx !== -1 && matchedIdx < subCmdIdx) {
        // Abaixo de Sub Comandante => aplica fallback (reserva)
        if (!gm.roles.cache.has(fallbackRoleId)) toAdd.push(fallbackRoleId);
