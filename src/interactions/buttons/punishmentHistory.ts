    ...
    async execute(interaction: ButtonInteraction) {
        try {
            ...
            if (customId.startsWith('punishment_history_summary:')) {
                ...
                const summaryEmbed = await punishmentHistoryService.createSummaryEmbed(executorId, interaction.client, interaction.guildId || undefined);
                ...
                const { embed, hasNextPage, hasPrevPage } = await punishmentHistoryService.createHistoryEmbed(executorId, interaction.client, page, 5, interaction.guildId || undefined);
                const navigationButtons = punishmentHistoryService.createNavigationButtons(executorId, page, hasNextPage, hasPrevPage);
                await interaction.editReply({
                    embeds: [embed],
                ...
                const executorId = customId.split(':')[1];
                await interaction.deferReply({ ephemeral: true });
                const stats = await punishmentHistoryService.getExecutorStatistics(executorId, interaction.guildId || undefined);
                let executorUser;
                try {
