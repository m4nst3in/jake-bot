...
export default {
    ...
    async execute(interaction: ButtonInteraction) {
        ...
        try {
            ...
            if (slot === 'merit') {
                if (!canSetMerit(interaction.member as GuildMember))
            ...
            if (ranks.length === 0)
                return interaction.editReply('Faixa de cargos indisponível.');
            ...
            if (row.components.length)
                rows.push(row);
            if (!rows.length)
                return interaction.editReply('Nenhum cargo configurado para essa faixa.');
