...
export default {
    ...
    async execute(interaction: ButtonInteraction) {
        ...
        if (!categoryId) {
            await interaction.reply({ content: 'Área inválida.', ephemeral: true });
            return;
        }
        ...
        modal.addComponents(new ActionRowBuilder<TextInputBuilder>().addComponents(texto), new ActionRowBuilder<TextInputBuilder>().addComponents(dim), new ActionRowBuilder<TextInputBuilder>().addComponents(cores), new ActionRowBuilder<TextInputBuilder>().addComponents(desc), new ActionRowBuilder<TextInputBuilder>().addComponents(entrega));
