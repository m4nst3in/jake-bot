...
export class RppSnapshotRepository extends BaseRepo {
    async upsert(userId: string, roles: string[]) {
        if (this.isSqlite()) {
            ...
            await new Promise<void>((resolve, reject) => {
                this.sqlite.run('INSERT INTO rpp_role_snapshots (user_id, roles, stored_at) VALUES (?,?,CURRENT_TIMESTAMP) ON CONFLICT(user_id) DO UPDATE SET roles=excluded.roles, stored_at=CURRENT_TIMESTAMP', [userId, rolesStr], function (err) { if (err)
                    reject(err);
                else
                    resolve(); });
            });
    ...
    async get(userId: string): Promise<RppSnapshotRecord | null> {
        if (this.isSqlite()) {
            return await new Promise<RppSnapshotRecord | null>((resolve, reject) => {
                this.sqlite.get('SELECT user_id, roles, stored_at FROM rpp_role_snapshots WHERE user_id=?', [userId], function (err, row: any) { if (err)
                    reject(err);
                else if (!row)
                    resolve(null);
                else
    ...
    async delete(userId: string) {
        if (this.isSqlite()) {
            await new Promise<void>((resolve, reject) => {
                this.sqlite.run('DELETE FROM rpp_role_snapshots WHERE user_id=?', [userId], function (err) { if (err)
                    reject(err);
                else
                    resolve(); });
            });
