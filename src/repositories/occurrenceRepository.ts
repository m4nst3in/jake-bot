...
export interface OccurrenceRecord {
  id?: number | string;
  staff_id: string; // usuário alvo
  motivo1: string; // título
  motivo2?: string; // detalhes
  resolucao: string; // ação tomada
  created_by: string; // quem registrou
  ...
  async add(rec: Omit<OccurrenceRecord, 'id' | 'created_at'>) {
    if (this.isSqlite()) {
      await new Promise<void>((resolve, reject) => {
        this.sqlite.run(
          'INSERT INTO occurrences (staff_id, motivo1, motivo2, resolucao, created_by, created_at) VALUES (?,?,?,?,?,CURRENT_TIMESTAMP)',
          [rec.staff_id, rec.motivo1, rec.motivo2 || null, rec.resolucao, rec.created_by],
  ...
  async countForUser(staffId: string) {
    if (this.isSqlite()) {
      return await new Promise<number>((resolve, reject) => {
        this.sqlite.get(
          'SELECT COUNT(*) c FROM occurrences WHERE staff_id=?',
          [staffId],
  ...
  async listRecentForUser(staffId: string, limit = 5) {
    if (this.isSqlite()) {
      return await new Promise<any[]>((resolve, reject) => {
        this.sqlite.all(
          'SELECT * FROM occurrences WHERE staff_id=? ORDER BY created_at DESC LIMIT ?',
          [staffId, limit],
