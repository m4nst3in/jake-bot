  ...
  cron.schedule('*/10 * * * *', async () => {
    try {
      ...
      if (eventsChannelId) {
        try {
          ...
          if (evCh && evCh.isTextBased()) {
            ...
            // manter mesmo emoji antes das posições já aplicado por buildRankingEmbedUnified; trocar setas/traço depois do usuário
            const adjusted = raw.split('\n').map((line:string)=>{
              if(!line.trim()) return line;
              return line.replace(/—/g, `${arrowEmoji}`); // substitui travessão por novo emoji
            }).join('\n');
            if((embed as any).data) (embed as any).data.description = adjusted; else (embed as any).setDescription(adjusted);
            // Cor roxa solicitada (#9B59BB)
        ...
        } catch(err){ logger.error({ err }, 'Falha ranking eventos'); }
      }
      ...
      if (journalismChannelId) {
        ...
        } catch(err){ logger.error({ err }, 'Falha ranking jornalismo'); }
      }
      ...
      try {
        ...
        if(dChannel && dChannel.isTextBased()){
          ...
          if((rankingEmbed as any).data) (rankingEmbed as any).data.description = cleaned; else (rankingEmbed as any).setDescription(cleaned);
      ...
      } catch(err){ logger.error({ err }, 'Falha ranking design'); }
    } catch(err){
