...
async function main() {
  ...
  if(!guildId) throw new Error('Nenhum guildId válido encontrado (configure mainGuildId ou support.guildId)');
  if(!process.env.DISCORD_TOKEN) throw new Error('DISCORD_TOKEN ausente');
  const client = new Client({ intents: [GatewayIntentBits.Guilds, GatewayIntentBits.GuildMembers] });
  await client.login(process.env.DISCORD_TOKEN);
  const guild = await client.guilds.fetch(guildId).catch(()=>null);
  if(!guild) throw new Error(`Guild principal não encontrada (id=${guildId})`);
  await guild.members.fetch();
  const role = guild.roles.cache.get(roleId);
  if(!role) throw new Error('Cargo staff não encontrado');
