...
sqlite3.verbose();
export type SqliteSchema = 'core' | 'points' | 'blacklist' | 'rpp';
export class SqliteAdapter {
    private db!: sqlite3.Database;
    constructor(private file: string, private schema: SqliteSchema = 'core') { }
    ...
    async migrate() {
        ...
        if (this.schema === 'points') {
            ...
            for (const stmt of stmts) { await this.run(stmt); }
            // de-dup and unique index as before
                    ...
                    for (const r of rows as any[]) {
                        await new Promise<void>((res2) => {
                            this.db.all('SELECT id, points, reports_count, shifts_count, last_updated FROM points WHERE user_id=? AND area=?', [r.user_id, r.area], async (err2, recs: any[]) => {
                                if (err2 || !recs?.length) return res2();
