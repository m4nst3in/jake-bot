...
interface Fonts { regular: string; bold: string; medium: string; }
...
function prepareFonts(doc: any): Fonts {
  ...
  (Object.keys(candidates) as (keyof Fonts)[]).forEach(k=>{
    for (const p of candidates[k]){
      if (existsSync(p)) { try { doc.registerFont(p, p); resolved[k] = p; } catch {} break; }
    }
  });
  return resolved;
}
...
export async function generateAreaPdf(client: Client, area: string): Promise<Buffer> {
  ...
  doc.fillColor('#fff').font(fonts.bold).fontSize(15).text(String(idx), badgeX+6, badgeY+8, { width:24, align:'center' });
    ...
    if (user){
      ...
      if (buf){
        try { doc.save().circle(badgeX+85, badgeY+45, 36).clip().image(buf, badgeX+49, badgeY+9, { width:72, height:72 }).restore(); avatarDrawn = true; } catch {}
      }
    }
    if (!avatarDrawn){
